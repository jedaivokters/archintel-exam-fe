<template>
    <div class="container">
        <div class="row">
            <div>
                <button class="btn btn-primary" @click="createForm">Create Article</button>
            </div>
...
<script>
export default {
    name: 'WriteArticleIndex',
    data() {
        return {
            writeId: this.$route.params.id ?? 0,
            writer: {},
            articlesEdits: [],
            articlesPublished: [],
        };
    ...
    async mounted() {
        let res = await this.$api.get(`users/${this.writeId}`);
        if (res.status === 200) {
            this.writer = res.data;

            if (this.writer.type !== 'Writer') {
    ...
    methods: {
        createForm() {
            this.$router.push({ name: 'writerCreateArticle' });
        },
        editorName(article) {
            if (!article.editor) {
                return '';
        ...
        editArticle(article) {
            this.$router.push({ name: 'writerArticleEdit', params: { articleId: article.id } });
        },
    },
};
</script>